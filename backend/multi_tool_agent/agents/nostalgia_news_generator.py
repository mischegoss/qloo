...
class NostalgiaNewsGenerator:
    """
    Agent 5: RESTORED WORKING Nostalgia News Generator - Original Structure + PII Fixes
    """
    ...
    def __init__(self, gemini_tool=None):
        ...
        logger.info("📰 Agent 5: RESTORED WORKING Nostalgia News Generator initialized")
        logger.info(f"🧠 Gemini tool available: {self.gemini_tool is not None}")
        logger.info("✅ Original FLAT structure with PII fixes guaranteed")
    ...
    def _load_theme_fallbacks(self) -> Dict[str, Any]:
        ...
        # Guaranteed fallbacks with newsletter-style content (PII-COMPLIANT)
        return {
            "music": {
                "memory_spotlight": "Music has the magical power to transport us through time and touch our hearts. Throughout history, melodies and songs have brought joy, comfort, and connection to people everywhere, creating soundtracks to our most precious memories.",
                "era_highlights": "Each era had its own special sound that defined the times. Radio brought music into homes and connected families to the wider world. Many families gathered around pianos for evening sing-alongs, creating memories that lasted a lifetime.",
                "heritage_traditions": "Every culture has its own musical heritage that tells the story of its people. Italian families might have treasured opera recordings, while Irish families passed down traditional folk songs that carried stories from generation to generation.",
            ...
            "food": {
                ...
                "era_highlights": "Those were the days when meals were prepared from scratch with care and attention. Picture grandmother's kitchen in the 1950s, with fresh bread rising and soup simmering on the stove. Every meal was a celebration of family togetherness and the simple pleasure of sharing good food.",
    ...
    def _extract_profile_data(self, agent1_output: Dict[str, Any]) -> Dict[str, Any]:
        """Extract profile data with safe defaults (PII-COMPLIANT)"""
        ...
        try:
            ...
            return {
                "first_name": "Friend",  # PII-COMPLIANT: Always use "Friend"
    ...
    async def _generate_with_gemini(self, profile_data: Dict[str, Any], content_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Generate using Gemini with NEWSLETTER TONE guidance (PII-COMPLIANT)"""
        ...
        if hasattr(self.gemini_tool, 'generate_nostalgia_newsletter'):
            try:
                ...
                # Create comprehensive newsletter prompt (PII-COMPLIANT)
                prompt = f"""
                ...
                # Define EXACT JSON structure (FLAT CONTENT - NOT NESTED)
                json_schema = {
        ...
        if hasattr(self.gemini_tool, 'generate_structured_json'):
            try:
                ...
                # Create comprehensive prompt with newsletter tone guidance (PII-COMPLIANT)
                prompt = f"""
    ...
    def _create_guaranteed_fallback(self, profile_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create guaranteed fallback content with newsletter-style tone (PII-COMPLIANT)"""
        ...
        if not theme_content:
            # Create newsletter-style content for any theme (PII-COMPLIANT)
    ...
    def _format_final_response(self, content: Dict[str, Any], profile_data: Dict[str, Any], source: str) -> Dict[str, Any]:
        """
        Format content into ORIGINAL WORKING FLAT STRUCTURE for Agent 6 (PII-COMPLIANT)
        
        CRITICAL: This maintains the EXACT structure the frontend expects
        """
        
        logger.info(f"🎨 Formatting ORIGINAL FLAT STRUCTURE final response from {source}")
        
        # CRITICAL: This is the EXACT FLAT structure the frontend expects
        final_response = {
            "title": f"Nostalgia News – {profile_data['current_date']}",
            "subtitle": f"{profile_data['theme_name']} Edition",
            "date": datetime.now().strftime("%B %d, %Y"),
            
            # THIS IS THE KEY - Agent 6 and frontend expect FLAT content structure
            "sections": {
                "memory_spotlight": {
            ...
            "metadata": {
                ...
                "pii_compliant": True,  # PII-COMPLIANT FLAG
                "anonymized": True      # ANONYMIZED FLAG
            }
        }
        
        # Log the structure we're creating
        logger.info(f"✅ ORIGINAL FLAT STRUCTURE final response created:")
        logger.info(f"   - Title: {final_response['title']}")
        logger.info(f"   - Sections: {list(final_response['sections'].keys())}")
        logger.info(f"   - Memory content type: {type(final_response['sections']['memory_spotlight']['content'])}")
        logger.info(f"   - Era content type: {type(final_response['sections']['era_highlights']['content'])}")
        logger.info(f"   - Heritage content type: {type(final_response['sections']['heritage_traditions']['content'])}")
        ...
        logger.info(f"   - Anonymized: {final_response['metadata']['anonymized']}")
    ...
    async def run(self,
                  ...
                  agent4c_output: Dict[str, Any]) -> Dict[str, Any]:
        """
        Generate nostalgia news with ORIGINAL WORKING FLAT STRUCTURE + PII COMPLIANCE
        """
        
        logger.info("📰 Agent 5: Generating ORIGINAL FLAT STRUCTURE + PII-COMPLIANT Nostalgia News")
        
        try:
            # Extract data with safe defaults (PII-COMPLIANT)
            ...
            # Format the final response with ORIGINAL WORKING FLAT STRUCTURE + PII compliance
            final_response = self._format_final_response(generated_content, profile_data, source)
            
            logger.info("✅ ORIGINAL FLAT STRUCTURE + PII-COMPLIANT nostalgia news generated!")
            ...
            logger.info(f"📊 Content types: memory={type(final_response['sections']['memory_spotlight']['content'])}, era={type(final_response['sections']['era_highlights']['content'])}")
            
            return {"nostalgia_news": final_response}
            
        except Exception as e:
            logger.error(f"❌ All methods failed, using emergency FLAT STRUCTURE fallback: {e}")
            
            # Emergency fallback with FLAT structure and PII compliance
            emergency_profile = {
                "first_name": "Friend",  # PII-COMPLIANT
